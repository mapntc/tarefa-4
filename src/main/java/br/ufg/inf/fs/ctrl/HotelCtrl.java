...
public class HotelCtrl {
	...
	@GetMapping("/estrelas/{qtdEstrela}")
	public ResponseEntity<List<Hotel>> findByQtdEstrela(@PathVariable Integer qtdEstrela){
		HttpHeaders headers = new HttpHeaders();
		HttpStatus status = HttpStatus.OK;
		List<Hotel> list = new ArrayList<Hotel>();
		try {
			list = business.findEstelas(qtdEstrela);
			if(list.size() == 0) {
				headers.add("message", Messages.get("0107"));
			}
		}catch (Exception e) {
	...
	public ResponseEntity<List<Hotel>> findByQtdEstrela(@PathVariable String nome){
		HttpHeaders headers = new HttpHeaders();
		HttpStatus status = HttpStatus.OK;
		List<Hotel> list = new ArrayList<Hotel>();
		try {
			list = business.findName(nome);
			if(list.size() == 0) {
				headers.add("message", Messages.get("0107"));
			}
		}catch (Exception e) {
